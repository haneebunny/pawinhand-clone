# 🐾 Pawinmatch (포인매치) - Pawinhand Clone
...
**Pawinmatch**는 이 고민을 3단계로 좁혀 실질적인 입양과 상담(행동 유도)까지 이어지도록 돕습니다.
...
### 1. 🩺 AI 입양 적합도 진단
- 주거 형태, 하루 외출시간, 산책 가능 시간 등 **6개 생활환경 지표**와 선호하는 아이의 활동성/사회성 성향을 입력받아 **"입양 가능 / 조건부 가능 / 신중히 재고"** 3단계 진단.
- RAG(참고문서 데이터 기반)를 활용해 룰 점수표가 아닌, 실제 파양 사유 및 양육 통계를 바탕으로 잘 맞는 점과 구체적인 보완 제안 제공.

### 2. 🎯 AI 맞춤 보호동물 매칭
- 진단 설문 내용과 103마리 보호동물의 성향 지표(활동량, 사회성, 건강상태, 크기 등)를 복합 분석하여 가장 잘 어울리는 **3~4마리의 추천견 매칭**.
- 카드 형태로 매칭 결과 목록을 보여주며, 각 카드마다 **직관적인 매칭 점수(1~10점)**와 **개별 맞춤 추천 이유 한 줄** 표시.

### 3. 🗺️ 보호동물 지도 시각화 (`/map`)
- 지리 정보 기반으로 지역별/보호소별 유기동물 보호 현황을 지도 위에 마커로 시각화.
- 지도의 핀을 누르면 해당 보호소의 이름, 주소, 연락처 등의 상세 정보와 함께 현재 보호 중인 아이들을 손쉽게 확인 가능.

### 4. 🗳️ 참여형 이름 추천 및 실시간 데이터 업데이트 (5표 누적 시 이름 확정)
- 이름이 정해지지 않아 `'이름 짓는 중!'`으로 분류된 아이들에게 사용자들이 직접 어울리는 이름을 지어주는 **참여형 작명 서비스**.
- 등록된 이름 추천 중 5표 이상의 투표(좋아요)를 획득하면, 백엔드/로컬의 `animals.json` 데이터가 해당 이름으로 **실시간 최종 확정 및 업데이트**되어 목록과 상세에 즉각 반영되는 인터랙티브 기능.

### 5. ⚡ Shimmer 애니메이션 기반 로딩 스켈레톤 (Skeleton UI)
- AI 매칭 분석을 대기할 때와 목록 필터(지역, 축종, 정렬)를 바꿀 때 실제 화면 구조를 정밀하게 복제한 **반짝이는 스켈레톤 카드** 노출.
- 로딩 찰나의 화면 끊김 현상을 극복하고 모바일 앱 수준의 매끄럽고 유기적인 데이터 전환 감성 제공.
